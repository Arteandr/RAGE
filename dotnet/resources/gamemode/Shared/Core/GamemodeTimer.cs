...
namespace Gamemode.Shared.Core
{
    public class GamemodeTimer
    {
        private static readonly List<GamemodeTimer> _timer = new List<GamemodeTimer>();

        private static readonly List<GamemodeTimer> _insertAfterList = new List<GamemodeTimer>();
        ...
        public GamemodeTimer(Action thefunc, uint executeafterms, uint executes = 1, bool handleexception = true)
        {
        ...
        public static void OnUpdateFunc()
        {
            ...
            lock (_timer)
            {
                for (int i = _timer.Count - 1; i >= 0; i--)
                {
                    ...
                    GamemodeTimer thetimer = _timer[i];
            ...
            lock (_insertAfterList)
            {
                ...
                {
                    foreach (GamemodeTimer timer in _insertAfterList)
                    {
                        timer.InsertSorted();
                    }
